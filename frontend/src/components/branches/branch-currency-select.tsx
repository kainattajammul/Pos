...
interface BranchCurrencySelectProps {
  value: string;
...
export function BranchCurrencySelect({
  value,
  onChange,
  id,
  disabled = false,
}: BranchCurrencySelectProps) {
  ...
  const options = getCurrencyOptions(value);
      ...
      ? createPortal(
          ...
          >
            ...
            <ul className="scrollbar-hide max-h-56 overflow-y-auto p-1.5">
              {filtered.length === 0 ? (
                <li className="px-3 py-6 text-center text-sm text-[#9CA3AF]">No currencies found</li>
              ) : (
                    ...
                    <li key={currency.code}>
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-[#F3F4F6] text-sm font-semibold text-[#374151]">
                            {currency.flag ?? currency.symbol}
                          </span>
                          <span className="min-w-0">
                            <span className="block truncate text-sm font-semibold">{currency.code}</span>
                            <span className="block truncate text-xs text-[#6B7280]">{currency.name}</span>
                          </span>
                        </span>
