...
import { MANAGE_SERVICES_TABS } from "@/lib/manage-services-tabs";
import { cn } from "@/lib/utils";

interface ManageServicesTabsProps {
  activeId?: string;
}

export function ManageServicesTabs({ activeId }: ManageServicesTabsProps) {
  const pathname = usePathname();

  return (
    <div className="overflow-x-auto border-b border-pos bg-pos-subtle">
      <div className="flex min-w-max px-2 pt-2">
        {MANAGE_SERVICES_TABS.map((tab) => {
          const isActive =
            activeId === tab.id ||
            pathname === tab.href ||
